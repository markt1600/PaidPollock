...
<title>The Atelier — Generative Studio</title>
<meta name="description" content="Generate museum-grade, high-resolution works in the manner of Jackson Pollock and Joan Miró, plus hand-embroidered silk carrés. Watch each commission happen live, frame it, download it.">
...
main{
  ...
  background:
    radial-gradient(58% 46% at 50% -6%, rgba(255,248,228,.5), rgba(255,248,228,0) 72%),
    radial-gradient(30% 26% at 26% -6%, rgba(255,246,224,.32), rgba(255,246,224,0) 75%),
    radial-gradient(30% 26% at 74% -6%, rgba(255,246,224,.32), rgba(255,246,224,0) 75%),
    radial-gradient(150% 120% at 50% 50%, var(--wall) 30%, var(--wall-deep) 100%);
  display:flex;align-items:center;justify-content:center;
  perspective:1500px;
  padding:42px 56px 172px;   /* room for the ledge below the story card */
  overflow:hidden auto;       /* a tall hang scrolls; nothing slides under the rail */
}
.hang{margin:auto 0}          /* keeps centring while allowing scroll overflow */
...
.hang{display:flex;flex-direction:column;align-items:center;gap:26px;max-width:100%;position:relative}
.hang>*{flex-shrink:0} /* a long label must never crush into the placard */
.hang::before{ /* the spot pools on the wall behind the piece */
  content:"";position:absolute;z-index:0;pointer-events:none;
  inset:-14% -18% 30% -18%;
  background:radial-gradient(60% 55% at 50% 28%, rgba(255,250,234,.22), rgba(255,250,234,0) 72%);
}
.framewrap{
  ...
  box-shadow:
    0 2px 4px rgba(25,18,8,.32),                 /* contact, near the wall */
    0 16px 26px -10px rgba(25,18,8,.42),
    0 46px 85px -18px rgba(25,18,8,.58);         /* thrown down the wall  */
  background:#0c0a07;
  transition:padding .2s;
  line-height:0;
}
.framewrap::before{ /* a warm halo that breathes while Claude is thinking */
  content:"";position:absolute;inset:-11px;z-index:0;pointer-events:none;
  border-radius:4px;
  box-shadow:0 0 0 3px rgba(222,174,92,.7), 0 0 52px 14px rgba(222,174,92,.5),
             0 0 110px 30px rgba(222,174,92,.22);
  opacity:0;
  transition:opacity .5s ease;
}
.framewrap.ai-thinking::before{
  opacity:1;
  animation:aiPulse 2s ease-in-out infinite;
}
@keyframes aiPulse{0%,100%{opacity:.4}50%{opacity:1}}
@property --aiA{syntax:'<angle>';inherits:false;initial-value:0deg;}
.ai-sparkle{ /* a glint orbiting the frame, with twinkles at its corners */
  position:absolute;inset:-12px;border-radius:5px;pointer-events:none;
  opacity:0;transition:opacity .5s ease;z-index:6;
}
.framewrap.ai-thinking .ai-sparkle{opacity:1}
.ai-sparkle::before{
  content:"";position:absolute;inset:0;border-radius:inherit;padding:3px;
  background:conic-gradient(from var(--aiA),
    transparent 0 8%, rgba(255,236,180,.95) 12%, rgba(255,214,130,.5) 15%, transparent 20%,
    transparent 46%, rgba(255,236,180,.65) 51%, transparent 57%);
...
@keyframes aiOrbit{to{--aiA:360deg}}
.ai-sparkle::after{
  content:"";position:absolute;inset:-4px;pointer-events:none;
  background:
    radial-gradient(circle 5px at 0% 0%, rgba(255,240,200,.95), transparent 70%),
    radial-gradient(circle 4px at 100% 0%, rgba(255,240,200,.9), transparent 70%),
    radial-gradient(circle 5px at 100% 100%, rgba(255,240,200,.95), transparent 70%),
    radial-gradient(circle 4px at 0% 100%, rgba(255,240,200,.9), transparent 70%),
    radial-gradient(circle 3px at 50% 0%, rgba(255,240,200,.8), transparent 70%),
    radial-gradient(circle 3px at 50% 100%, rgba(255,240,200,.8), transparent 70%);
  animation:aiTwinkle 1.6s ease-in-out infinite;
}
@keyframes aiTwinkle{0%,100%{opacity:.2}50%{opacity:1}}
@media (prefers-reduced-motion: reduce){
  .framewrap.ai-thinking::before{animation:none;opacity:.75}
  .ai-sparkle::before{animation:none}
  .ai-sparkle::after{animation:none;opacity:.6}
}
.framewrap::after{ /* the room's light falls from above */
  content:"";position:absolute;inset:0;z-index:3;pointer-events:none;
  background:linear-gradient(to bottom,
    rgba(255,250,232,.075),
    rgba(255,250,232,0) 24%,
    rgba(20,14,6,0) 68%,
    rgba(20,14,6,.10));
}
.framewrap .liner{background:#eee8d8;transition:padding .2s;line-height:0}
.framewrap.silkliner .liner{background:linear-gradient(135deg,#f6f1e5 0%,#efe9da 45%,#f7f2e7 100%)}
...
/* curator's note beneath the placard */
.story{
  background:var(--placard);color:#4a4337;
  max-width:min(440px,calc(100vw - 32px));
  padding:11px 18px 12px;border-radius:1px;
  box-shadow:0 8px 22px -8px rgba(25,18,8,.4);
  font-family:'Marcellus',serif;font-size:13px;line-height:1.55;
  font-style:italic;text-align:left;
}
.story::before{content:"— ";color:#9a8f78}
@media (max-width:880px){
  ...
  main{
    padding:26px max(12px,env(safe-area-inset-right)) 30px max(12px,env(safe-area-inset-left));
    min-height:70vh;min-height:72dvh;
    overflow:visible;                      /* grow with the page instead of clipping */
  }
  .rail{position:static;margin-top:30px}   /* in flow: nothing can hide beneath it */
...
<aside>
  <div class="brand">
    <h1>The<br>Atelier <em>·</em></h1>
    <p>An algorithmic studio. Commission a Jackson Pollock pour — flung, dripped and pooled in enamel — a Joan Miró dream painted in ink, crayon and oil, or a hand-embroidered silk carré. All finished at archival resolution.</p>
  </div>

  <div class="field">
    <label id="lab-coll">Collection</label>
    <div class="chips" role="group" aria-labelledby="lab-coll" id="collChips"></div>
  </div>

  <div class="field" id="fieldFormat">
  ...
  <div class="field" id="fieldMotif" hidden>
    <label id="lab-motif">Motif</label>
    <div class="chips" role="group" aria-labelledby="lab-motif" id="motifChips"></div>
  </div>
  ...
  <div class="field">
    <label id="lab-ai">AI art director</label>
    <div class="chips" role="group" aria-labelledby="lab-ai" id="aiChips"></div>
  </div>
  ...
  <p class="fineprint">Paintings are thrown over about three seconds — physics-simulated pours under real gravity. Dream paintings are worked in ink, wax crayon and oil. Carrés are stitched thread by thread with hand-rolled hems. Everything renders at up to 4,400&nbsp;px; downloads include your chosen frame. No two commissions repeat.</p>
</aside>
...
<main>
  <div class="hang">
    <div class="framewrap f-none" id="framewrap">
      ...
      <div class="ai-sparkle" aria-hidden="true"></div>
    </div>
    <div class="placard" id="placard">
      ...
      <div class="artist">The Atelier</div>
...
/* ---------- state ---------- */
const state={mode:"pollock",format:"classic",palette:"convergence",scarfPalette:"flamme",miroPalette:"reve",motif:"chaine",frame:"none",seed:0,frameSeed:1,dyn:.6,title:"",activeId:null,done:false,
...
function oblob(ctx,cx,cy,r,squash,ang,scrub){
  ...
  /* paint body: ridge the edge, settle the middle */
  for(let i=0;i<pts.length;i+=2)hstamp(pts[i][0],pts[i][1],r*.16,.12);
  for(let i=0;i<10;i++)hstamp(cx+R(-.5,.5)*r,cy+R(-.5,.5)*r*sq,r*.3,.05);
  return pts;
}
function odisc(ctx,cx,cy,r){
  ...
  for(let i=0;i<8;i++){
    const a=i*.785;
    hstamp(cx+Math.cos(a)*r*.85,cy+Math.sin(a)*r*.85,r*.2,.1);
  }
  hstamp(cx,cy,r*.5,.05);
}
function oring(ctx,cx,cy,r,w){
  ...
  for(let i=0;i<=n;i++){
    ...
    if((i&1)===0)hstamp(cx+Math.cos(a)*r,cy+Math.sin(a)*r,w*.8,.08);
  }
}
...
function omoon(ctx,cx,cy,r,ang){
  ...
  for(let i=0;i<pts.length;i+=4){
    const x=cx+pts[i][0]*ca-pts[i][1]*sa, y=cy+pts[i][0]*sa+pts[i][1]*ca;
    hstamp(x,y,r*.14,.1);
  }
}
...
function startGeneration(opts={}){
  ...
  if(entry){ /* rehang a work from the ledge */
    ...
    state.mode=entry.mode==="scarf"?"scarf":entry.mode==="miro"?"miro":"pollock";
    if(state.mode==="scarf"){
      state.format="square";
  ...
  const SC=state.mode==="scarf", MI=state.mode==="miro";
  ...
  document.getElementById("pDims").textContent=
    SC?"90 × 90 cm, hand-rolled hem":FORMATS[state.format].dims;
  markActiveThumb();
  ...
  (async()=>{
    const aiLive=state.ai&&aiAvailable&&!entry&&!reduce&&!SC;
...
function syncModeUI(){
  ...
  document.getElementById("regen").textContent=
    SC?"Commission a new carré":MI?"Paint a new dream":"Pour a new painting";
