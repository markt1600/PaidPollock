...
- **Geometry and repetition** — gardens are now composed the way real
  carrés are: most commissions roll four-fold rotational symmetry (or a
  two-fold turn), with every layer — canopy, crowns, mid growth, accent
  clusters, sprigs — sown in one arm and repeated identically about the
  centre, plus a garland ring of matching blooms and a grand bloom
  holding the exact middle; corner spills are always identical at all
  four corners; only the occasional commission grows free. The repeated
  arms replay the same random stream, so the repetition is exact.
